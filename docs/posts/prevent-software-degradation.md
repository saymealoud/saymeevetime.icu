...
*   **Open for Extension (对扩展开放)**：通过新增模块、类或方法来扩展功能。
*   **Closed for Modification (对修改封闭)**：不修改已有的、经过测试的稳定代码。
...
| ❌ 错误方式 | ✅ 正确方式 |
| :--- | :--- |
| 直接修改原有代码，堆砌 if-else 分支 | 通过接口/抽象类定义契约 |
| 每次需求变更都触碰核心逻辑 | 新功能通过实现接口扩展，原有代码无需改动 |
...
1.  **🎩 重构的帽子**：不改变软件可观察行为，只调整内部结构。提取方法、重命名、抽象接口，为新功能铺路。
2.  **🧢 新功能的帽子**：添加新的用户行为。在重构良好的基础上，新功能实现应顺理成章——添加类、实现接口。
...
*   **违背 OCP**：任何逻辑变更都需修改 OrderService，系统极不稳定。
*   **破坏封装**：数据与行为分离，任何 Service 都可随意修改 Entity。
*   **依赖地狱**：Service 注入大量 DAO，处理本不该关心的细节。
*   **面向过程**：本质是披着 OOP 外衣的 Procedural Code。
...
*   **符合真实世界**：对象即现实，订单包含与之相关的规则约束。
*   **高内聚**：库存归 Inventory，优惠归 Coupon，职责清晰。
*   **可测试性**：直接对领域对象编写单元测试，无需 Spring 容器。
*   **消除依赖地狱**：Service 不再注入大量 DAO，通过聚合根协作。
...
1.  **时刻重构**：戴好重构帽，添加新功能前先清理战场。
2.  **拥抱 OCP**：用扩展代替修改，隔离变化点。
3.  **回归对象**：采用充血模型，让 Entity 承载业务逻辑。
